---
title: 用eslint & prettier & pre-commit规范前端代码格式
date: 2019-09-20 15:34:34
tags:
---
...
```
{
  ...
  "useTabs": "false", // 是否使用tab替换空格
  "semi": "true", // 是否在每句末尾添加分号
  "singleQuote": "false", // 字符串使用单引号替代双引号
  "jsxSingleQuote": "false", // jsx语法中是否使用单引号代替双引号
   ...
   "rangeStart": <int>, // 仅格式化文件从<位置>开始, 默认 0
   rangeEnd: <int>, // 仅格式化文件从<位置>结束 默认 Infinity
   "parser": "babylon", // 代码的解析引擎，默认为babylon，与babel相同。
   "filepath": "<string>", // 指定用于推断要使用的解析器的文件名。
   "requirePragma": <bool>, // 是否在被格式化文件开头添加注释
   "insertPragma": <bool>, // 使用@format标记向开头添加换行符。
}
```
更多配置规则：![prettier配置规则](https://prettier.io/docs/en/options.html)
